  ...
  if msg.from.type == 'user' then
    ...
    if msgs > max_msg then
	  ...
	  if msg.to.type == 'user' then
		...
		if msgs >= max_msg then
			print("Pass2")
			send_large_msg("user#id"..msg.from.id, "<b>User ["..msg.from.id.."] blocked for spam.</b>")
			savelog(msg.from.id.." PM", "<i>User ["..msg.from.id.."] blocked for spam.</i>")
	  ...
	  if msg.to.type == 'chat' or msg.to.type == 'channel' then
		if username then
			...
			send_large_msg(receiver , "<b>Flooding is not allowed here\n@"..username.."["..msg.from.id.."]\nStatus: User kicked</b")
		else
      ...
      if gbanspamonredis then
        if tonumber(gbanspamonredis) ==  4 and not is_owner(msg) then
		  ...
		  for k,v in pairs(GBan_log) do
			log_SuperGroup = v
			gban_text = "<b>User [ "..name.." ] ( @"..username.." )"..msg.from.id.." Globally banned from ( "..msg.to.print_name.." ) [ "..msg.to.id.." ] (spamming)</b>"
