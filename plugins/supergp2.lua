...
local title ="<b>Info for SuperGroup: ["..result.title.."]\n\n</b>"
local admin_num = "<b>Admin count: "..result.admins_count.."\n</b>"
local user_num = "<b>User count: "..result.participants_count.."\n</b>"
...
local function lock_group_etehad(msg, data, target)
  ...
  if group_etehad_lock == 'yes' then
    return '<b>supergp setting is already locked</b>'
  else
...
local function get_rules(msg, data)
  ...
  local rules = group_name..'<b> rules:\n\n'..rules:gsub("/n‌</b>", " ")
  return rules
end
...
local function run(msg, matches)
	...
	local data = load_data(_config.moderation.data)
		...
		unlock_group_operator(msg, data, target),
      	...
      	return lock_group_super(msg, data, target), super
      end
		...
		unlock_group_operator(msg, data, target),
      	...
      	return unlock_group_super(msg, data, target), dsuper
      end
			if matches[2] == 'links' then
